...
import { validateGraph } from './graph-validation.ts';
...
export function validateCatalogue(
  ...
  for (const r of [...d.techniques, ...d.publications]) validateVerification(r);
  unique(
  ...
  for (const t of d.techniques) {
    for (const field of [
      ...
      if (
        JSON.stringify(t[field]) !== JSON.stringify(t.taxonomy.general[field])
      )
        throw new Error(
          'General taxonomy compatibility fields must match canonical technique fields',
        );
    if (
      t.introducedYear !== null &&
      t.introductionStatus !== 'original-publication-confirmed'
    )
      throw new Error(
        'Introduction year requires original-publication-confirmed status',
      );
    if (
      t.introductionStatus === 'original-publication-confirmed' &&
      t.introducedYear === null
    )
      throw new Error('Confirmed introduction requires a year');
    if (
      t.introductionStatus === 'earliest-identified' &&
      t.earliestIdentifiedYear === null
    )
      throw new Error('Earliest identified status requires a year');
    if (
      t.introducedYear !== null &&
      t.earliestIdentifiedYear !== null &&
      t.introducedYear > t.earliestIdentifiedYear
    )
      throw new Error(
        'Introduction cannot postdate earliest identified discussion',
      );
    for (const [key, task] of [
      ...
      if (t.taxonomy[key] && !t.tasks.includes(task))
        throw new Error('Task-specific taxonomy requires supported task');
  }
  validateGraph(d, options.references !== false);
...
export function assertRelations(d: Catalogue): void {
  ...
  for (const x of d.techniques) {
    ...
    x.implementations.forEach((i) => evidence(i.scientificBasis, x.id));
...
export function mergeCatalogue(
  ...
  const merged: Catalogue = {
    schemaVersion: 3,
    claims: [...current.claims, ...batch.claims],
    publicationRelationships: [
      ...current.publicationRelationships,
      ...batch.publicationRelationships,
    ],
    publicationSimilarities: [
      ...current.publicationSimilarities,
      ...batch.publicationSimilarities,
    ],
    curationDecisions: [
      ...current.curationDecisions,
      ...batch.curationDecisions,
    ],
    candidateLiterature: [
      ...current.candidateLiterature,
      ...batch.candidateLiterature,
    ],
...
export function buildSearchIndex(d: Catalogue): Map<string, string> {
  ...
  for (const l of d.techniquePublications) {
    ...
    list.push(
      (pub.get(l.publicationId) ?? '') +
        ' ' +
        (l.notes ?? '') +
        ' ' +
        l.evidence.map((e) => e.notes || '').join(' '),
    );
    ...
    (t) =>
      ...
      (f.environment === 'all' ||
        (f.environment === 'unknown'
          ? t.taxonomy.general.environment === null
          : t.taxonomy.general.environment?.some(
              (v) => v === f.environment,
            ))) &&
      (f.distance === 'all' ||
        (f.distance === 'unknown'
...
export function comparisonData(d: Catalogue, ids: string[]) {
  ...
  const fields: [string, (t: Technique) => unknown][] = [
    ...
    ['Environment', (t) => t.taxonomy.general.environment],
    ['Target properties', (t) => t.taxonomy.selection?.targetProperties],
    ['Selection mechanism', (t) => t.taxonomy.selection?.selectionMechanism],
    ['Control mapping', (t) => t.taxonomy.general.controlMapping],
    ['Confirmation method', (t) => t.taxonomy.selection?.confirmationMethod],
    ['Introduction year', (t) => t.introducedYear],
    ['Earliest identified year', (t) => t.earliestIdentifiedYear],
    ['Introduction status', (t) => t.introductionStatus],
    ['Manipulation taxonomy', (t) => t.taxonomy.manipulation],
    ['Navigation taxonomy', (t) => t.taxonomy.navigation],
    ['System control taxonomy', (t) => t.taxonomy.systemControl],
    [
