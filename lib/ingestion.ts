...
export interface Curation {
  ...
  publicationRelationships?: Catalogue['publicationRelationships'];
  claims?: Catalogue['claims'];
  candidateLiterature?: Catalogue['candidateLiterature'];
...
export function ingestLiterature(
  ...
  for (const t of curation.techniques) {
    if (t.verificationStatus !== 'machine-curated')
      throw new Error('Extracted candidates must be machine-curated.');
    if (
  ...
  for (const relation of curation.publicationRelationships ?? []) {
    if (relation.verificationStatus === 'human-verified')
      throw new Error('Ingestion cannot promote human verification');
    d.publicationRelationships.push({
      ...relation,
      status: 'needs-evidence',
      sourcePublicationId: canonical(relation.sourcePublicationId),
      targetPublicationId: canonical(relation.targetPublicationId),
      evidence: evidence(relation.evidence),
      provenance: provenance(relation.provenance),
    });
  }
  for (const claim of curation.claims ?? []) {
    if (
      claim.verificationStatus === 'human-verified' ||
      claim.status === 'confirmed'
    )
      throw new Error('Ingestion cannot confirm human claims');
    d.claims.push({ ...claim, evidence: evidence(claim.evidence) });
  }
  d.candidateLiterature.push(...(curation.candidateLiterature ?? []));
  return {
    catalogue: validateCatalogue(normalizeVerification(d)),
    report: {
      publicationsAdded,
      publicationsEnriched,
      duplicatesAvoided,
